            ...
            const TreeOpts& o)
{
  ...
  if (prune && out > 0)
    {
      // The risk a node carries as a leaf: the weight its majority class does
      // not hold, or the squared error about its mean.
      std::vector<double> risk (out);
      for (octave_idx_type i = 0; i < out; i++)
        {
          if (isreg)
            risk[i] = nodeerror(i) * nodeweight(i);
          else
            {
              double best = 0.0;
              for (octave_idx_type k = 0; k < K; k++)
                best = std::max (best, classweight(i, k));
              risk[i] = nodeweight(i) - best;
            }
        }
  ...
  octave_scalar_map T;
  T.assign ("PruneList", prunelist);
  T.assign ("PruneAlpha", prunealpha);
