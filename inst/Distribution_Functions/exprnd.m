...
function r = exprnd (mu, varargin)
  ...
  elseif (nargin > 2)
    ...
    if (any (notint))
      error ("exprnd: dimensions must be integers.");
    endif
    sz = [varargin{:}];
  endif
...
## Test output
%!assert_equal (size (exprnd (2)), [1, 1])
%!assert_equal (size (exprnd (ones (2,1))), [2, 1])
%!assert_equal (size (exprnd (ones (2,2))), [2, 2])
%!assert_equal (size (exprnd (1, 3)), [3, 3])
%!assert_equal (size (exprnd (1, [4 1])), [4, 1])
%!assert_equal (size (exprnd (1, 4, 1)), [4, 1])
%!assert_equal (size (exprnd (1, 4, 1)), [4, 1])
%!assert_equal (size (exprnd (1, 4, 1, 5)), [4, 1, 5])
%!assert_equal (size (exprnd (1, 0, 1)), [0, 1])
%!assert_equal (size (exprnd (1, 1, 0)), [1, 0])
%!assert_equal (size (exprnd (1, 1, 2, 0, 5)), [1, 2, 0, 5])
%!assert_equal (size (exprnd (1, [])), [0, 0])
%!assert_equal (size (exprnd (1, [2, 0, 2, 1])), [2, 0, 2])
%!assert_equal (size (exprnd (1, -1)), [0, 0])
%!assert_equal (size (exprnd (1, [2, -1, 2])), [2, 0, 2])
%!assert_equal (size (exprnd (1, 2, -1, 5)), [2, 0, 5])

## Test class of input preserved
%!assert_equal (class (exprnd (2)), "double")
%!assert_equal (class (exprnd (single (2))), "single")
%!assert_equal (class (exprnd (single ([2 2]))), "single")

## Test input validation
%!error<exprnd: function called with too few input arguments.> exprnd ()
%!error<exprnd: MU must not be complex.> exprnd (i)
%!error<exprnd: SZ must be a scalar or a row vector of integers.> ...
%! exprnd (1, 1.2)
%!error<exprnd: SZ must be a scalar or a row vector of integers.> ...
%! exprnd (1, ones (2))
%!error<exprnd: SZ must be a scalar or a row vector of integers.> ...
