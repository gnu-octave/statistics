...
function y = nbinpdf (x, r, ps)
  ...
  if (! (isnumeric (x) && isnumeric (r) && isnumeric (ps)))
    error ("nbinpdf: X, R, and PS must be double, single, or integer.");
  endif
...
%!error<nbinpdf: X, R, and PS must be double, single, or integer.> nbinpdf (true, 2, 2)
%!error<nbinpdf: X, R, and PS must be double, single, or integer.> nbinpdf ('a', 2, 2)
%!assert_equal (class (nbinpdf (int32 (2), 2, 2)), 'double')
