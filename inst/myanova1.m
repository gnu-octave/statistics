...
## -*- texinfo -*-
## @deftypefn  {statistics} {@var{p} =} myanova1 (@var{x})
## @deftypefnx {statistics} {@var{p} =} myanova1 (@var{x}, @var{group})
## @deftypefnx {statistics} {@var{p} =} myanova1 (@var{x}, @var{group}, @var{displayopt})
## @deftypefnx {statistics} {@var{p} =} myanova1 (@var{x}, @var{group}, @var{displayopt}, @var{vartype})
## @deftypefnx {statistics} {[@var{p}, @var{atab}] =} myanova1 (@var{x}, @dots{})
## @deftypefnx {statistics} {[@var{p}, @var{atab}, @var{stats}] =} myanova1 (@var{x}, @dots{})
##
...
##
## myanova1 can take up to three input arguments:
##
## @itemize
## @item
...
## @end itemize
##
## myanova1 can return up to three output arguments:
##
## @itemize
## @item
...
## @end itemize
##
## If myanova1 is called without any output arguments, then it prints the results
...
function [p, anovatab, stats] = myanova1 (x, group, displayopt, vartype)

  ## Check for valid number of input arguments
  if (nargin < 1 || nargin > 4)
    error ("myanova1: invalid number of input arguments.");
  endif
  ## Add defaults
  if (nargin < 2)
    group = [];
  ...
  if (length (x) < prod (size (x)))
    ...
    elseif (size (group, 1) == m)     ## group names exist and match columns
      ...
      error ("myanova1: columns in X and GROUP length do not match.");
    endif
  endif

  ## Check that x and group are the same size
  if (! all (numel (x) == numel (group)))
    error ("myanova1: GROUP must be a vector with the same number of rows as x.");
  endif
  ...
  if (SSE != 0)                     ## Regular Matrix case.
    switch (lower (vartype))
      ...
      otherwise
        error ("myanova1: invalid fourth (vartype) argument to myanova1.");
    endswitch
  ...
  if (plotdata)
    myboxplot (x, group_id, "Notch", "on", "Labels", group_names, "Orientation", "vertical");
  endif
endfunction
...
%! y = [54 87 45; 23 98 39; 45 64 51; 54 77 49; 45 89 50; 47 NaN 55];
%! g = [1  2  3 ; 1  2  3 ; 1  2  3 ; 1  2  3 ; 1  2  3 ; 1  2  3 ];
%! [p, tbl] = myanova1 (y(:), g(:), "off", "equal");
%! assert (p, 0.00004163, 1e-6);
%! assert (tbl{2,5}, 22.573418, 1e-6);
%! assert (tbl{2,3}, 2, 0);
%! assert (tbl{3,3}, 14, 0);
%! [p, tbl] = myanova1 (y(:), g(:), "off", "unequal");
