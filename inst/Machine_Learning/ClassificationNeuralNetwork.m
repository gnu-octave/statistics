...
classdef ClassificationNeuralNetwork < PredictiveModel
  ## -*- texinfo -*-
  ...
  methods (Access = public)
    ...
    ## @deftypefnx {ClassificationNeuralNetwork} {@var{m} =} margin (@var{obj}, @var{Tbl}, @var{ResponseVarName})
    ## @deftypefnx {ClassificationNeuralNetwork} {@var{m} =} margin (@var{obj}, @var{Tbl})
    ##
...
## came out of it.  The field list is MATLAB's where the quantity is the same,
## with OutputLayerActivation, Solver and LearningRate added, which MATLAB has
## no counterpart for.
%!test
%! load fisheriris
%! Mdl = fitcnet (meas, species);
...
%!test
%! load fisheriris
%! MP = fitcnet (meas, species).ModelParameters;
%! assert_equal (MP.LayerSizes, 10);
%! assert_equal (MP.Activations, 'relu');
%! assert_equal (MP.OutputLayerActivation, 'softmax');
%! assert_equal (MP.Solver, 'LBFGS');
%! assert_equal (MP.IterationLimit, 1000);
%! assert_equal (MP.GradientTolerance, 1e-6);
%! assert_equal (MP.LossTolerance, 1e-6);
%! assert_equal (MP.StepTolerance, 1e-6);
...
## Every value tracks the argument that set it, and a per-layer Activations
## stays a cellstr rather than splitting the structure into an array.
%!test
%! load fisheriris
%! MP = fitcnet (meas, species, 'LayerSizes', [5, 3], 'Activations', ...
%!               {'relu', 'tanh'}, 'Standardize', true, ...
%!               'IterationLimit', 50).ModelParameters;
%! assert_equal (size (MP), [1, 1]);
%! assert_equal (MP.LayerSizes, [5, 3]);
%! assert_equal (MP.Activations, {'relu', 'tanh'});
%! assert_equal (MP.StandardizeData, true);
%! assert_equal (MP.IterationLimit, 50);
...
## A rectifier takes He and a symmetric activation takes Glorot, so a network
## whose layers differ is built with both.
%!test
%! load fisheriris
%! Mdl = fitcnet (meas, species, 'LayerSizes', [5, 3], ...
