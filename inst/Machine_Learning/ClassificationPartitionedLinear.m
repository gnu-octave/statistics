...
classdef ClassificationPartitionedLinear
  ...
  properties (GetAccess = public, SetAccess = protected)
    ...
    ## SVM reports @qcode{Method} as @qcode{'PartitionedLinear'} and not
    ## @qcode{'SVM'}.
    ##
  ...
  methods (Access = public)
    ...
    function this = ClassificationPartitionedLinear (X, Y, varargin)
      ...
      if (istable (X))
        ...
        if (! isempty (errmsg))
          error ("ClassificationPartitionedLinear: %s", errmsg);
        endif
      endif
    ...
    function m = kfoldMargin (this)
      ...
      if (! isempty (errmsg))
        error ("ClassificationPartitionedLinear.kfoldMargin: %s", errmsg);
      endif
    ...
    function e = kfoldEdge (this, varargin)
      ...
      [gY, errmsg] = labelIndices (this.ClassNames, this.Y);
      if (! isempty (errmsg))
        error ("ClassificationPartitionedLinear.kfoldEdge: %s", errmsg);
      endif
      m = marginsOf (s, gY, this.NumLambda_);
    ...
    function l = kfoldLoss (this, varargin)
      ...
      [gY, errmsg] = labelIndices (this.ClassNames, this.Y);
      if (! isempty (errmsg))
        error ("ClassificationPartitionedLinear.kfoldLoss: %s", errmsg);
      endif
  ...
  methods (Access = public, Hidden)
    ...
    function this = set.ScoreTransform (this, val)
      [f, nm] = parseScoreTransform (val, 'ClassificationPartitionedLinear');
...
%! load fisheriris
%! part = cvpartition (species(51:end), 'KFold', 5);
%! plain = ClassificationPartitionedLinear (meas(51:end,:), species(51:end), ...
%!                                          'CVPartition', part);
%! CVMdl = ClassificationPartitionedLinear (meas(51:end,:), species(51:end), ...
%!                                          'CVPartition', part, ...
...
%!error<ClassificationPartitionedLinear: unrecognized 'ScoreTransform' function.> ...
%! load fisheriris
%! CVMdl = ClassificationPartitionedLinear (meas(51:end,:), species(51:end), ...
%!                                          'KFold', 5);
...
%!test
%! load fisheriris
%! Mdl = fitclinear (meas, strcmp (species, 'setosa'), 'KFold', 3);
...
%! CVMdl = ClassificationPartitionedLinear (T, 'Species', 'KFold', 3);
%! assert_equal (class (CVMdl), 'ClassificationPartitionedLinear');
