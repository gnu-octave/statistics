...
classdef CompactClassificationNeuralNetwork < PredictiveModel
  ## -*- texinfo -*-
  ...
  methods (Hidden)
    ...
    function this = set.Cost (this, val)
      ...
      if (isempty (val))
        ...
        if (! isempty (errmsg))
          error ("CompactClassificationNeuralNetwork: %s", errmsg);
        endif
        this.Cost = val;
      endif
    endfunction
  ...
  methods (Access = public)
    ...
    function [labels, scores] = predict (this, XC)
      ...
      XC = tableColumns (this, 'CompactClassificationNeuralNetwork.predict', XC);
    ...
    ## @deftypefnx {CompactClassificationNeuralNetwork} {@var{m} =} margin (@var{obj}, @var{Tbl}, @var{ResponseVarName})
    ## @deftypefnx {CompactClassificationNeuralNetwork} {@var{m} =} margin (@var{obj}, @var{Tbl})
    ##
    ...
    ## @deftypefnx {CompactClassificationNeuralNetwork} {@var{e} =} edge (@var{obj}, @var{Tbl}, @var{ResponseVarName})
    ## @deftypefnx {CompactClassificationNeuralNetwork} {@var{e} =} edge (@var{obj}, @var{Tbl})
    ##
    ...
    ## @deftypefnx {CompactClassificationNeuralNetwork} {@var{L} =} loss (@var{obj}, @var{Tbl}, @var{ResponseVarName})
    ## @deftypefnx {CompactClassificationNeuralNetwork} {@var{L} =} loss (@var{obj}, @var{Tbl})
...
%! delete (fname);
%! assert_equal (CMdl2.LayerWeights, CMdl.LayerWeights);
%! assert_equal (CMdl2.LayerBiases, CMdl.LayerBiases);
...
%! [~, s1] = predict (CMdl2, [1, 2; 4, 5]);
%! [~, s2] = predict (compact (Mdl), [1, 2; 4, 5]);
%! assert_equal (s1, s2);
...
%!error<CompactClassificationNeuralNetwork: the number of rows and columns in 'Cost' must correspond to selected classes in Y.> ...
...
%!test
%! load fisheriris
%! Mdl = compact (fitcnet (meas, species));
...
%!error<CompactClassificationNeuralNetwork: 'Cost' must have zeros on its diagonal.> ...
%! load fisheriris
%! Mdl = compact (fitcnet (meas, species));
%! Mdl.Cost = ones (3);
...
%! CMdl = compact (fitcnet (T, 'Species'));
