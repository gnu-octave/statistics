...
classdef CompactClassificationGAM < PredictiveModel
  ## -*- texinfo -*-
  ...
  methods (Hidden)
    ...
    function this = set.Cost (this, val)
      ...
      if (isempty (val))
        ...
        if (! isempty (errmsg))
          error ("CompactClassificationGAM: %s", errmsg);
        endif
        this.Cost = val;
      endif
    endfunction
  ...
  methods (Access = public)
    ...
    function [labels, scores] = predict (this, XC, varargin)
      ...
      XC = tableColumns (this, 'CompactClassificationGAM.predict', XC);
    ...
    ## @deftypefnx {CompactClassificationGAM} {@var{m} =} margin (@var{obj}, @var{Tbl}, @var{ResponseVarName})
    ## @deftypefnx {CompactClassificationGAM} {@var{m} =} margin (@var{obj}, @var{Tbl})
    ##
    ...
    ## @deftypefnx {CompactClassificationGAM} {@var{e} =} edge (@var{obj}, @var{Tbl}, @var{ResponseVarName})
    ## @deftypefnx {CompactClassificationGAM} {@var{e} =} edge (@var{obj}, @var{Tbl})
    ##
...
%!error<CompactClassificationGAM: 'Cost' must have zeros on its diagonal.> ...
%! load fisheriris
%! inds = ! strcmp (species, 'virginica');
%! Mdl = compact (fitcgam (meas(inds,:), species(inds)));
%! Mdl.Cost = ones (2);
...
%!test
%! load fisheriris
%! Mdl = compact (fitcgam (meas, strcmp (species, 'setosa')));
...
%! CMdl = compact (fitcgam (T, 'Species'));
