...
classdef CompactRegressionTree < PredictiveModel
  ## -*- texinfo -*-
  ...
  methods (Access = public)
    ...
    ## @deftypefnx {CompactRegressionTree} {@var{L} =} loss (@var{obj}, @var{Tbl}, @var{ResponseVarName})
    ## @deftypefnx {CompactRegressionTree} {@var{L} =} loss (@var{obj}, @var{Tbl})
...
%! means = [3, 1, 4, 1.5, 5];
%! y = means(c)' + 0.1 * sin (k);
%! Full = fitrtree (c, y, 'CategoricalPredictors', 1, 'MaxNumSplits', 2);
%! Mdl = compact (Full);
%! assert_equal (Mdl.CutCategories, Full.CutCategories);
%! assert_equal (predict (Mdl, [1; 2; 6]), predict (Full, [1; 2; 6]));
