...
classdef CompactRegressionSVM < PredictiveModel
  ...
  methods (Access = public)
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {CompactRegressionSVM} {@var{obj} =} discardSupportVectors (@var{obj})
    ##
    ...
    function yFit = predict (this, XC)
      ...
      XC = tableColumns (this, 'CompactRegressionSVM.predict', XC);
    ...
    ## @deftypefnx {CompactRegressionSVM} {@var{L} =} loss (@var{obj}, @var{Tbl}, @var{ResponseVarName})
    ## @deftypefnx {CompactRegressionSVM} {@var{L} =} loss (@var{obj}, @var{Tbl})
...
%! Mdl = compact (fitrsvm (X, y, "KernelFunction", "linear"));
%! D = discardSupportVectors (Mdl);
%! assert_equal (class (D), "CompactRegressionSVM");
...
%! keep = ! strcmp (species, "setosa");
%! X = meas(keep,2:4); y = meas(keep,1);
%! discardSupportVectors (compact (fitrsvm (X, y, "KernelFunction", "rbf")))
...
%! X = meas(:,2:4);
%! Y = meas(:,1);
%! Mdl = compact (fitrsvm (X, Y));
%! fname = tempname ();
%! savemodel (Mdl, fname);
%! M2 = loadmodel (fname);
%! delete (fname);
%! assert_equal (class (M2), 'CompactRegressionSVM');
...
%!test
%! load fisheriris
%! Mdl = compact (fitrsvm (meas(:,2:4), meas(:,1)));
