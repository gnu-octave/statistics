...
classdef ClassificationSVM < PredictiveModel
  ## -*- texinfo -*-
  ...
  methods (Access = public)
    ...
    ## @deftypefnx {ClassificationSVM} {@var{m} =} margin (@var{obj}, @var{Tbl}, @var{ResponseVarName})
    ## @deftypefnx {ClassificationSVM} {@var{m} =} margin (@var{obj}, @var{Tbl})
    ##
    ...
    ## @deftypefnx {ClassificationSVM} {@var{e} =} edge (@var{obj}, @var{Tbl}, @var{ResponseVarName})
    ## @deftypefnx {ClassificationSVM} {@var{e} =} edge (@var{obj}, @var{Tbl})
    ##
...
%! X = meas(51:150,1:2);
%! y = species(51:150);
%! a = loss (fitcsvm (X, y), X, y);
%! yc = categorical (y);
%! assert_equal (loss (fitcsvm (X, yc), X, yc), a);
%! ys = string (y);
%! assert_equal (loss (fitcsvm (X, ys), X, ys), a);

%!test  # margin reads one as it reads a cellstr response
%! load fisheriris
%! X = meas(51:150,1:2);
%! y = species(51:150);
%! a = margin (fitcsvm (X, y), X, y);
%! yc = categorical (y);
%! assert_equal (margin (fitcsvm (X, yc), X, yc), a);
%! ys = string (y);
%! assert_equal (margin (fitcsvm (X, ys), X, ys), a);
