...
classdef cvpartition
  ...
  methods (Access = public)
    ...
    ## @var{c}.
    ##
    ## This method calculates the distribution of classes (if stratified) or groups
    ## (if grouped) across the entire dataset, as well as within every training
    ## and test set generated by the partition.
    ##
    ## @subheading Inputs
    ## @itemize
    ## @item @var{c}
    ...
    ## @enumerate
    ## @item The partition type (@code{c.Type}) must be @qcode{"kfold"} or @qcode{"holdout"}.
    ## @item The partition must be created with a stratification or grouping variable
    ## (i.e., @code{c.IsStratified} or @code{c.IsGrouped} must be @code{true}).
    ## @end enumerate
    ## @end itemize
    ##
    ## @subheading Outputs
    ## @itemize
    ## @item @var{tbl}
    ## A @code{table} object containing the summary statistics. The table contains
    ## one row for every unique label/group in every set (all, train, test).
    ## The columns are:
    ## @table @code
    ## @item Set
    ## The specific subset being described. Values include @qcode{"all"} (the full
    ## dataset), @qcode{"train1"}, @qcode{"test1"}, etc.
    ## @item SetSize
    ## The total number of observations in that specific set.
    ## @item Label
    ## The class or group identifier. If @code{c.IsStratified} is true, this column is
    ## named @code{StratificationLabel}. If @code{c.IsGrouped} is true, it is named
    ## @code{GroupLabel}.
    ## @item Count
    ## The number of observations of that label within the set. If stratified, this
    ## column is named @code{StratificationCount}; otherwise, @code{GroupCount}.
    ## @item PercentInSet
    ...
    ##
    ## @subheading Example
    ## Create a stratified 3-fold partition for a simple dataset and display the summary:
    ##
    ## @example
    ## @group
    ## species = [repmat({"Setosa"}, 10, 1); ...
    ##            repmat({"Versicolor"}, 10, 1); ...
    ##            repmat({"Virginica"}, 10, 1)];
    ...
    function tbl = summary (this)
      ...
      if (iscell (UniqueLabels) || isstring (UniqueLabels) || ischar (UniqueLabels))
