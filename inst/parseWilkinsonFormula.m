  ...
  while (i <= n)
    ...
    switch (c)
      ...
      case ')'
        type = "RPAREN";
      otherwise
        error ("parseWilkinsonFormula: Unexpected character '%s' at position %d", c, i);
    endswitch
  ...
  if (strcmp (t.type, "IDENTIFIER"))
    ...
    if (curr <= n && strcmp (tokens(curr).type, "LPAREN"))
      ...
      if (curr <= n && strcmp (tokens(curr).type, "RPAREN"))
        ...
        error ("parseWilkinsonFormula: Missing ')' for function call '%s'.", func_name);
      endif
...
function [X, y, col_names] = run_model_matrix_builder (schema, data)
  ...
  if (isempty (req_vars))
    ...
    if (! check_has_var (req_vars{1}))
      error ("parseWilkinsonFormula: Unknown variable '%s' in Data Table.", req_vars{1});
    endif
  ...
  if (! isempty (schema.ResponseIdx))
    ...
    if (! check_has_var (y_name))
      error ("parseWilkinsonFormula: Unknown variable '%s' in Data Table.", y_name);
    endif
    ...
    for k = 1:length (y_vars)
      ...
      if (iscell (raw_y))
          ...
          try
            ...
            error ("parseWilkinsonFormula: Response variable '%s' must be numeric.", y_name);
          end_try_catch
      else
          col_data = raw_y(valid_mask, :);
      endif
      ...
      if (size (col_data, 1) != n_rows)
        error ("parseWilkinsonFormula: Mismatch in number of rows for response variable '%s'", y_name);
      endif
...
function vars = resolve_lhs_vars (lhs_str, data)
  ...
  for i = 1:length (parts)
    ...
    if (length (range_parts) == 2)
      ...
      if (! found_s), error ("parseWilkinsonFormula: Unknown variable '%s' in range", start_var); endif
      if (! found_e), error ("parseWilkinsonFormula: Unknown variable '%s' in range", end_var); endif
    ...
    elseif (length (range_parts) == 1)
      ...
      if (! any (strcmp (all_names, p)))
        error ("parseWilkinsonFormula: Response variable '%s' not found in Data.", p);
      endif
      vars = [vars, {p}];
    else
