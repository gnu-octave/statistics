...
classdef InverseGaussianDistribution < prob.ProbabilityDistribution
  ## -*- texinfo -*-
  ## @deftp {statistics} prob.InverseGaussianDistribution
  ##
  ## Inverse Gaussian probability distribution object.
  ##
  ## A @code{prob.InverseGaussianDistribution} object consists of parameters, a
  ...
  ##
  ## There are several ways to create a @code{prob.InverseGaussianDistribution}
  ## object.
  ##
  ## @itemize
  ...
  properties(Dependent = true)
    ## -*- texinfo -*-
    ## @deftp {prob.InverseGaussianDistribution} {property} mu
    ##
    ## Mean parameter
    ##
    ## A positive scalar value characterizing the mean of the
    ## Inverse Gaussian distribution. You can access the @qcode{mu}
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.InverseGaussianDistribution} {property} lambda
    ##
    ## Shape parameter
    ##
    ## A positive scalar value characterizing the shape of the
    ## Inverse Gaussian distribution. You can access the @qcode{lambda}
    ## property using dot name assignment.
    ##
    ## @end deftp
    lambda
  ...
  properties(GetAccess = public, Constant = true)
    ## -*- texinfo -*-
    ## @deftp {prob.InverseGaussianDistribution} {property} DistributionName
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.InverseGaussianDistribution} {property} NumParameters
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.InverseGaussianDistribution} {property} ParameterNames
    ##
    ## Names of parameters
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.InverseGaussianDistribution} {property} ParameterDescription
    ##
  ...
  properties(GetAccess = public, SetAccess = protected)
    ## -*- texinfo -*-
    ## @deftp {prob.InverseGaussianDistribution} {property} ParameterValues
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.InverseGaussianDistribution} {property} ParameterIsFixed
    ##
  ...
  methods(Hidden)
    ...
    function this = InverseGaussianDistribution (mu, lambda)
      ...
      checkparams (mu, lambda);
      this.InputData = [];
      this.IsTruncated = false;
      this.ParameterValues = [mu, lambda];
  ...
  methods(Access = public)
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {@var{p} =} cdf (@var{pd}, @var{x})
    ## @deftypefnx {prob.InverseGaussianDistribution} {@var{p} =} cdf (@var{pd}, @var{x}, @qcode{'upper'})
    ##
    ...
    function p = cdf (this, x, uflag)
      ...
      ## Do the computations
      p = invgcdf (x, this.mu, this.lambda);
      if (this.IsTruncated)
        ...
        p(lb) = 0;
        p(ub) = 1;
        p(! (lb | ub)) -= invgcdf (lx, this.mu, this.lambda);
        p(! (lb | ub)) /= diff (invgcdf ([lx, ux], this.mu, this.lambda));
      endif
      ## Apply uflag
      if (utail)
        p = 1 - p;
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {@var{x} =} icdf (@var{pd}, @var{p})
    ##
    ...
    function x = icdf (this, p)
      ...
      if (this.IsTruncated)
        lp = invgcdf (this.Truncation(1), this.mu, this.lambda);
        up = invgcdf (this.Truncation(2), this.mu, this.lambda);
        ## Adjust p values within range of p @ lower limit and p @ upper limit
        is_nan = p < 0 | p > 1;
        p(is_nan) = NaN;
        np = lp + (up - lp) .* p;
        x = invginv (np, this.mu, this.lambda);
        x(x < this.Truncation(1)) = this.Truncation(1);
        x(x > this.Truncation(2)) = this.Truncation(2);
      else
        x = invginv (p, this.mu, this.lambda);
      endif
    endfunction

    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {@var{r} =} iqr (@var{pd})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {@var{m} =} mean (@var{pd})
    ##
    ...
    function m = mean (this)
      ...
      if (this.IsTruncated)
        ...
        m = invgstat (this.mu, this.lambda);
      endif
    endfunction

    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {@var{m} =} median (@var{pd})
    ##
    ...
    function m = median (this)
      ...
      if (this.IsTruncated)
        lx = this.Truncation(1);
        ux = this.Truncation(2);
        Fa_b = invgcdf ([lx, ux], this.mu, this.lambda);
        m = invginv (sum (Fa_b) / 2, this.mu, this.lambda);
      else
        m = invginv (0.5, this.mu, this.lambda);
      endif
    endfunction

    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {@var{nlogL} =} negloglik (@var{pd})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {@var{ci} =} paramci (@var{pd})
    ## @deftypefnx {prob.InverseGaussianDistribution} {@var{ci} =} paramci (@var{pd}, @var{Name}, @var{Value})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {@var{y} =} pdf (@var{pd}, @var{x})
    ##
    ...
    function y = pdf (this, x)
      ...
      y = invgpdf (x, this.mu, this.lambda);
      if (this.IsTruncated)
        lx = this.Truncation(1);
        lb = x < lx;
        ux = this.Truncation(2);
        ub = x > ux;
        y(lb | ub) = 0;
        y(! (lb | ub)) /= diff (invgcdf ([lx, ux], this.mu, this.lambda));
      endif
    endfunction

    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {} plot (@var{pd})
    ## @deftypefnx {prob.InverseGaussianDistribution} {} plot (@var{pd}, @var{Name}, @var{Value})
    ## @deftypefnx {prob.InverseGaussianDistribution} {@var{h} =} plot (@dots{})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd}, @var{pnum})
    ## @deftypefnx {prob.InverseGaussianDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd}, @var{pnum}, @qcode{'Display'}, @var{display})
    ## @deftypefnx {prob.InverseGaussianDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd}, @var{pnum}, @var{setparam})
    ## @deftypefnx {prob.InverseGaussianDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd}, @var{pnum}, @var{setparam}, @qcode{'Display'}, @var{display})
    ## @deftypefnx {prob.InverseGaussianDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd})
    ## @deftypefnx {prob.InverseGaussianDistribution} {[@var{nlogL}, @var{param}, @var{other}] =} proflik (@dots{})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {@var{r} =} random (@var{pd})
    ## @deftypefnx {prob.InverseGaussianDistribution} {@var{r} =} random (@var{pd}, @var{rows})
    ## @deftypefnx {prob.InverseGaussianDistribution} {@var{r} =} random (@var{pd}, @var{rows}, @var{cols}, @dots{})
    ## @deftypefnx {prob.InverseGaussianDistribution} {@var{r} =} random (@var{pd}, [@var{sz}])
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {@var{t} =} truncate (@var{pd}, @var{lower}, @var{upper})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.InverseGaussianDistribution} {@var{v} =} var (@var{pd})
    ##
    ...
    function v = var (this)
      ...
      if (this.IsTruncated)
        ...
        [~, v] = invgstat (this.mu, this.lambda);
      endif
    endfunction
  ...
  methods(Static, Hidden)
    ...
    function pd = fit (x, varargin)
      ...
      [~, acov] = invglike (phat, x, censor, freq);
      ## Create fitted distribution object
      pd = prob.InverseGaussianDistribution.makeFitted ...
...
function checkparams (mu, lambda)
...
## Test output
%!shared pd, t
%! pd = prob.InverseGaussianDistribution (1, 1);
%! t = truncate (pd, 2, 4);
...
## 'prob.InverseGaussianDistribution' constructor
%!error <InverseGaussianDistribution: MU must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution (0, 1)
%!error <InverseGaussianDistribution: MU must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution (Inf, 1)
%!error <InverseGaussianDistribution: MU must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution (i, 1)
%!error <InverseGaussianDistribution: MU must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution ('a', 1)
%!error <InverseGaussianDistribution: MU must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution ([1, 2], 1)
%!error <InverseGaussianDistribution: MU must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution (NaN, 1)
%!error <InverseGaussianDistribution: LAMBDA must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution (1, 0)
%!error <InverseGaussianDistribution: LAMBDA must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution (1, -1)
%!error <InverseGaussianDistribution: LAMBDA must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution (1, Inf)
%!error <InverseGaussianDistribution: LAMBDA must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution (1, i)
%!error <InverseGaussianDistribution: LAMBDA must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution (1, 'a')
%!error <InverseGaussianDistribution: LAMBDA must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution (1, [1, 2])
%!error <InverseGaussianDistribution: LAMBDA must be a positive real scalar.> ...
%! prob.InverseGaussianDistribution (1, NaN)

## 'cdf' method
%!error <cdf: invalid argument for upper tail.> ...
%! cdf (prob.InverseGaussianDistribution, 2, 'uper')
%!error <cdf: invalid argument for upper tail.> ...
%! cdf (prob.InverseGaussianDistribution, 2, 3)

## 'paramci' method
%!shared x
%! x = invgrnd (1, 1, [1, 100]);
%!error <paramci: optional arguments must be in NAME-VALUE pairs.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'alpha')
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'alpha', 0)
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'alpha', 1)
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'alpha', [0.5 2])
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'alpha', '')
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'alpha', {0.05})
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'parameter', 'mu', ...
%!          'alpha', {0.05})
%!error <paramci: invalid VALUE size for 'Parameter' argument.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), ...
%!          'parameter', {'mu', 'lambda', 'param'})
%!error <paramci: invalid VALUE size for 'Parameter' argument.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'alpha', 0.01, ...
%!          'parameter', {'mu', 'lambda', 'param'})
%!error <paramci: unknown distribution parameter.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'parameter', 'param')
%!error <paramci: unknown distribution parameter.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'alpha', 0.01, ...
%!          'parameter', 'param')
%!error <paramci: invalid NAME for optional argument.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'NAME', 'value')
%!error <paramci: invalid NAME for optional argument.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'alpha', 0.01, 'NAME', 'value')
%!error <paramci: invalid NAME for optional argument.> ...
%! paramci (prob.InverseGaussianDistribution.fit (x), 'alpha', 0.01, ...
...
%! plot (prob.InverseGaussianDistribution, 'Parent')
%!error <plot: invalid VALUE for 'PlotType' argument.> ...
%! plot (prob.InverseGaussianDistribution, 'PlotType', 12)
%!error <plot: invalid VALUE size for 'Parameter' argument.> ...
%! plot (prob.InverseGaussianDistribution, 'PlotType', {'pdf', 'cdf'})
%!error <plot: invalid VALUE for 'PlotType' argument.> ...
%! plot (prob.InverseGaussianDistribution, 'PlotType', 'pdfcdf')
%!error <plot: invalid VALUE for 'Discrete' argument.> ...
%! plot (prob.InverseGaussianDistribution, 'Discrete', 'pdfcdf')
%!error <plot: invalid VALUE for 'Discrete' argument.> ...
%! plot (prob.InverseGaussianDistribution, 'Discrete', [1, 0])
%!error <plot: invalid VALUE for 'Discrete' argument.> ...
%! plot (prob.InverseGaussianDistribution, 'Discrete', {true})
%!error <plot: invalid VALUE for 'Parent' argument.> ...
%! plot (prob.InverseGaussianDistribution, 'Parent', 12)
%!error <plot: invalid VALUE for 'Parent' argument.> ...
%! plot (prob.InverseGaussianDistribution, 'Parent', 'hax')
%!error <plot: invalid NAME for optional argument.> ...
%! plot (prob.InverseGaussianDistribution, 'invalidNAME', 'pdf')
%!error <plot: no fitted DATA to plot a probability plot.> ...
%! plot (prob.InverseGaussianDistribution, 'PlotType', 'probability')

## 'proflik' method
%!error <proflik: no fitted data available.> ...
%! proflik (prob.InverseGaussianDistribution, 2)
%!error <proflik: PNUM must be a scalar number indexing a non-fixed parameter.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), 3)
%!error <proflik: PNUM must be a scalar number indexing a non-fixed parameter.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), [1, 2])
%!error <proflik: PNUM must be a scalar number indexing a non-fixed parameter.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), {1})
%!error <proflik: SETPARAM must be a numeric vector.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), 1, ones (2))
%!error <proflik: missing VALUE for 'Display' argument.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), 1, 'Display')
%!error <proflik: invalid VALUE type for 'Display' argument.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), 1, 'Display', 1)
%!error <proflik: invalid VALUE type for 'Display' argument.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), 1, 'Display', {1})
%!error <proflik: invalid VALUE type for 'Display' argument.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), 1, 'Display', {'on'})
%!error <proflik: invalid VALUE size for 'Display' argument.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), 1, 'Display', ['on'; 'on'])
%!error <proflik: invalid VALUE for 'Display' argument.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), 1, 'Display', 'onnn')
%!error <proflik: invalid NAME for optional arguments.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), 1, 'NAME', 'on')
%!error <proflik: invalid optional argument.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), 1, {'NAME'}, 'on')
%!error <proflik: invalid optional argument.> ...
%! proflik (prob.InverseGaussianDistribution.fit (x), 1, {[1 2 3]}, 'Display', 'on')

## 'truncate' method
%!error <truncate: missing input argument.> ...
%! truncate (prob.InverseGaussianDistribution)
%!error <truncate: missing input argument.> ...
%! truncate (prob.InverseGaussianDistribution, 2)
%!error <truncate: invalid lower upper limits.> ...
%! truncate (prob.InverseGaussianDistribution, 4, 2)

## Catch errors when using array of probability objects with available methods
%!shared pd
%! pd = prob.InverseGaussianDistribution (1, 1);
%! pd(2) = prob.InverseGaussianDistribution (1, 3);
