...
## Input validation of Y, the weights and the optional arguments
%!error <nlinfit: Y must be a real numeric vector.> ...
%! nlinfit ([1;2;3;4], {1, 2}, @(b, x) b(1) * x, 1)
%!error <nlinfit: RobustWgtFun must be a character vector.> ...
%! nlinfit ([1;2;3;4], [2;4;6;8], @(b, x) b(1) * x, 1, "RobustWgtFun", 5)
%!error <nlinfit: WEIGHTS must be a real numeric vector.> ...
%! nlinfit ([1;2;3;4], [2;4;6;8], @(b, x) b(1) * x, 1, "Weights", {1, 2, 3, 4})
%!error <nlinfit: WEIGHTS must have one element per observation.> ...
%! nlinfit ([1;2;3;4], [2;4;6;8], @(b, x) b(1) * x, 1, "Weights", [1; 2])
%!error <nlinfit: WEIGHTS must be nonnegative.> ...
%! nlinfit ([1;2;3;4], [2;4;6;8], @(b, x) b(1) * x, 1, "Weights", [1; -1; 1; 1])
%!error <nlinfit: WEIGHTS must be nonnegative.> ...
%! nlinfit ([1;2;3;4], [2;4;6;8], @(b, x) b(1) * x, 1, ...
%!          "Weights", [1; NaN; 1; 1])
%!error <nlinfit: Name/Value arguments must come in pairs.> ...
%! nlinfit ([1;2;3;4], [2;4;6;8], @(b, x) b(1) * x, 1, "Weights")
%!error <nlinfit: parameter names must be character vectors.> ...
%! nlinfit ([1;2;3;4], [2;4;6;8], @(b, x) b(1) * x, 1, 42, 1)
%!error <nlinfit: OPTIONS must be a structure.> ...
%! nlinfit ([1;2;3;4], [2;4;6;8], @(b, x) b(1) * x, 1, ...
%!          struct ("MaxIter", {10, 20}))

## Edge cases with empty arrays
%!error <nlinfit: Y must be a vector.> ...
%! nlinfit (ones (4, 2), ones (2, 2), @(b, x) b(1) * x, 1)
%!error <nlinfit: MODELFUN must return a vector of the same size as Y.> ...
