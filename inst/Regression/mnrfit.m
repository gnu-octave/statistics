...
function [B, DEV, STATS] = mnrfit (X, Y, varargin)
  ...
  if (isempty (Y))
    error ("mnrfit: X and Y must contain at least one valid observation.");
  endif
  if (isnumeric (Y))
...
%!fail ("mnrfit ([], [])", ...
%!      "mnrfit: X and Y must contain at least one valid observation.")
%!fail ("mnrfit (zeros(0,3), zeros(0,1))", ...
%!      "mnrfit: X and Y must contain at least one valid observation.")
