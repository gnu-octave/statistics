...
classdef GammaDistribution < prob.ProbabilityDistribution
  ## -*- texinfo -*-
  ## @deftp {statistics} prob.GammaDistribution
  ##
  ## Gamma probability distribution object.
  ##
  ## A @code{prob.GammaDistribution} object consists of parameters, a model
  ...
  ##
  ## There are several ways to create a @code{prob.GammaDistribution} object.
  ##
  ## @itemize
  ...
  ## @item Use the constructor @qcode{prob.GammaDistribution (@var{a}, @var{b})}
  ## to create a gamma distribution with fixed parameter values @var{a} and
  ## @var{b}.
  ## @item Use the static method @qcode{prob.GammaDistribution.fit (@var{x},
  ...
  properties(Dependent = true)
    ## -*- texinfo -*-
    ## @deftp {prob.GammaDistribution} {property} a
    ##
    ## Shape parameter
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.GammaDistribution} {property} b
    ##
    ## Scale parameter
    ##
  ...
  properties(GetAccess = public, Constant = true)
    ## -*- texinfo -*-
    ## @deftp {prob.GammaDistribution} {property} DistributionName
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.GammaDistribution} {property} NumParameters
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.GammaDistribution} {property} ParameterNames
    ##
    ## Names of parameters
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.GammaDistribution} {property} ParameterDescription
    ##
  ...
  properties(GetAccess = public, SetAccess = protected)
    ## -*- texinfo -*-
    ## @deftp {prob.GammaDistribution} {property} ParameterValues
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.GammaDistribution} {property} ParameterCovariance
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.GammaDistribution} {property} ParameterIsFixed
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.GammaDistribution} {property} Truncation
    ##
    ## Truncation interval
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.GammaDistribution} {property} IsTruncated
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.GammaDistribution} {property} InputData
    ##
  ...
  methods(Access = public)
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {@var{p} =} cdf (@var{pd}, @var{x})
    ## @deftypefnx {prob.GammaDistribution} {@var{p} =} cdf (@var{pd}, @var{x}, @qcode{'upper'})
    ##
    ...
    function p = cdf (this, x, uflag)
      ...
      if (this.IsTruncated)
        ...
        p(lb) = 0;
        p(ub) = 1;
        p(! (lb | ub)) -= gamcdf (lx, this.a, this.b);
        p(! (lb | ub)) /= diff (gamcdf ([lx, ux], this.a, this.b));
      endif
      ## Apply uflag
      if (utail)
        p = 1 - p;
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {@var{x} =} icdf (@var{pd}, @var{p})
    ##
    ...
    function x = icdf (this, p)
      ...
      if (this.IsTruncated)
        lp = gamcdf (this.Truncation(1), this.a, this.b);
        up = gamcdf (this.Truncation(2), this.a, this.b);
        ## Adjust p values within range of p @ lower limit and p @ upper limit
        is_nan = p < 0 | p > 1;
        p(is_nan) = NaN;
        np = lp + (up - lp) .* p;
        x = gaminv (np, this.a, this.b);
        x(x < this.Truncation(1)) = this.Truncation(1);
        x(x > this.Truncation(2)) = this.Truncation(2);
      else
        x = gaminv (p, this.a, this.b);
      endif
    endfunction

    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {@var{r} =} iqr (@var{pd})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {@var{m} =} mean (@var{pd})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {@var{m} =} median (@var{pd})
    ##
    ...
    function m = median (this)
      ...
      if (this.IsTruncated)
        lx = this.Truncation(1);
        ux = this.Truncation(2);
        Fa_b = gamcdf ([lx, ux], this.a, this.b);
        m = gaminv (sum (Fa_b) / 2, this.a, this.b);
      else
        m = gaminv (0.5, this.a, this.b);
      endif
    endfunction

    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {@var{nlogL} =} negloglik (@var{pd})
    ##
    ...
    function nlogL = negloglik (this)
      ...
      nlogL = gamlike ([this.a, this.b], this.InputData.data, ...
                         this.InputData.cens, this.InputData.freq);
    endfunction

    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {@var{ci} =} paramci (@var{pd})
    ## @deftypefnx {prob.GammaDistribution} {@var{ci} =} paramci (@var{pd}, @var{Name}, @var{Value})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {@var{y} =} pdf (@var{pd}, @var{x})
    ##
    ...
    function y = pdf (this, x)
      ...
      if (this.IsTruncated)
        ...
        ub = x > ux;
        y(lb | ub) = 0;
        y(! (lb | ub)) /= diff (gamcdf ([lx, ux], this.a, this.b));
      endif
    endfunction

    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {} plot (@var{pd})
    ## @deftypefnx {prob.GammaDistribution} {} plot (@var{pd}, @var{Name}, @var{Value})
    ## @deftypefnx {prob.GammaDistribution} {@var{h} =} plot (@dots{})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd}, @var{pnum})
    ## @deftypefnx {prob.GammaDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd}, @var{pnum}, @qcode{'Display'}, @var{display})
    ## @deftypefnx {prob.GammaDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd}, @var{pnum}, @var{setparam})
    ## @deftypefnx {prob.GammaDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd}, @var{pnum}, @var{setparam}, @qcode{'Display'}, @var{display})
    ## @deftypefnx {prob.GammaDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd})
    ## @deftypefnx {prob.GammaDistribution} {[@var{nlogL}, @var{param}, @var{other}] =} proflik (@dots{})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {@var{r} =} random (@var{pd})
    ## @deftypefnx {prob.GammaDistribution} {@var{r} =} random (@var{pd}, @var{rows})
    ## @deftypefnx {prob.GammaDistribution} {@var{r} =} random (@var{pd}, @var{rows}, @var{cols}, @dots{})
    ## @deftypefnx {prob.GammaDistribution} {@var{r} =} random (@var{pd}, [@var{sz}])
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {@var{t} =} truncate (@var{pd}, @var{lower}, @var{upper})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.GammaDistribution} {@var{v} =} var (@var{pd})
    ##
  ...
  methods(Static, Hidden)
    ...
    function pd = fit (x, varargin)
      ...
      [~, acov] = gamlike (phat, x, censor, freq);
      ## Create fitted distribution object
      pd = prob.GammaDistribution.makeFitted (phat, pci, acov, x, censor, freq);
    endfunction
...
## 'prob.GammaDistribution' constructor
%!error <GammaDistribution: A must be a positive real scalar.> ...
%! prob.GammaDistribution (0, 1)
%!error <GammaDistribution: A must be a positive real scalar.> ...
%! prob.GammaDistribution (Inf, 1)
%!error <GammaDistribution: A must be a positive real scalar.> ...
%! prob.GammaDistribution (i, 1)
%!error <GammaDistribution: A must be a positive real scalar.> ...
%! prob.GammaDistribution ('a', 1)
%!error <GammaDistribution: A must be a positive real scalar.> ...
%! prob.GammaDistribution ([1, 2], 1)
%!error <GammaDistribution: A must be a positive real scalar.> ...
%! prob.GammaDistribution (NaN, 1)
%!error <GammaDistribution: B must be a positive real scalar.> ...
%! prob.GammaDistribution (1, 0)
%!error <GammaDistribution: B must be a positive real scalar.> ...
%! prob.GammaDistribution (1, -1)
%!error <GammaDistribution: B must be a positive real scalar.> ...
%! prob.GammaDistribution (1, Inf)
%!error <GammaDistribution: B must be a positive real scalar.> ...
%! prob.GammaDistribution (1, i)
%!error <GammaDistribution: B must be a positive real scalar.> ...
%! prob.GammaDistribution (1, 'a')
%!error <GammaDistribution: B must be a positive real scalar.> ...
%! prob.GammaDistribution (1, [1, 2])
%!error <GammaDistribution: B must be a positive real scalar.> ...
%! prob.GammaDistribution (1, NaN)

## 'cdf' method
%!error <cdf: invalid argument for upper tail.> ...
%! cdf (prob.GammaDistribution, 2, 'uper')
%!error <cdf: invalid argument for upper tail.> ...
%! cdf (prob.GammaDistribution, 2, 3)

## 'paramci' method
%!shared x
%! x = gamrnd (1, 1, [100, 1]);
%!error <paramci: optional arguments must be in NAME-VALUE pairs.> ...
%! paramci (prob.GammaDistribution.fit (x), 'alpha')
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.GammaDistribution.fit (x), 'alpha', 0)
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.GammaDistribution.fit (x), 'alpha', 1)
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.GammaDistribution.fit (x), 'alpha', [0.5 2])
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.GammaDistribution.fit (x), 'alpha', '')
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.GammaDistribution.fit (x), 'alpha', {0.05})
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.GammaDistribution.fit (x), 'parameter', 'a', 'alpha', {0.05})
%!error <paramci: invalid VALUE size for 'Parameter' argument.> ...
%! paramci (prob.GammaDistribution.fit (x), 'parameter', {'a', 'b', 'param'})
%!error <paramci: invalid VALUE size for 'Parameter' argument.> ...
%! paramci (prob.GammaDistribution.fit (x), 'alpha', 0.01, ...
%!          'parameter', {'a', 'b', 'param'})
%!error <paramci: unknown distribution parameter.> ...
%! paramci (prob.GammaDistribution.fit (x), 'parameter', 'param')
%!error <paramci: unknown distribution parameter.> ...
%! paramci (prob.GammaDistribution.fit (x), 'alpha', 0.01, 'parameter', 'param')
%!error <paramci: invalid NAME for optional argument.> ...
%! paramci (prob.GammaDistribution.fit (x), 'NAME', 'value')
%!error <paramci: invalid NAME for optional argument.> ...
%! paramci (prob.GammaDistribution.fit (x), 'alpha', 0.01, 'NAME', 'value')
%!error <paramci: invalid NAME for optional argument.> ...
%! paramci (prob.GammaDistribution.fit (x), 'alpha', 0.01, 'parameter', 'a', ...
%!          'NAME', 'value')

## 'plot' method
%!error <plot: optional arguments must be in NAME-VALUE pairs.> ...
%! plot (prob.GammaDistribution, 'Parent')
%!error <plot: invalid VALUE for 'PlotType' argument.> ...
%! plot (prob.GammaDistribution, 'PlotType', 12)
%!error <plot: invalid VALUE size for 'Parameter' argument.> ...
%! plot (prob.GammaDistribution, 'PlotType', {'pdf', 'cdf'})
%!error <plot: invalid VALUE for 'PlotType' argument.> ...
%! plot (prob.GammaDistribution, 'PlotType', 'pdfcdf')
%!error <plot: invalid VALUE for 'Discrete' argument.> ...
%! plot (prob.GammaDistribution, 'Discrete', 'pdfcdf')
%!error <plot: invalid VALUE for 'Discrete' argument.> ...
%! plot (prob.GammaDistribution, 'Discrete', [1, 0])
%!error <plot: invalid VALUE for 'Discrete' argument.> ...
%! plot (prob.GammaDistribution, 'Discrete', {true})
%!error <plot: invalid VALUE for 'Parent' argument.> ...
%! plot (prob.GammaDistribution, 'Parent', 12)
%!error <plot: invalid VALUE for 'Parent' argument.> ...
%! plot (prob.GammaDistribution, 'Parent', 'hax')
%!error <plot: invalid NAME for optional argument.> ...
%! plot (prob.GammaDistribution, 'invalidNAME', 'pdf')
%!error <plot: no fitted DATA to plot a probability plot.> ...
%! plot (prob.GammaDistribution, 'PlotType', 'probability')

## 'proflik' method
%!error <proflik: no fitted data available.> ...
%! proflik (prob.GammaDistribution, 2)
%!error <proflik: PNUM must be a scalar number indexing a non-fixed parameter.> ...
%! proflik (prob.GammaDistribution.fit (x), 3)
%!error <proflik: PNUM must be a scalar number indexing a non-fixed parameter.> ...
%! proflik (prob.GammaDistribution.fit (x), [1, 2])
%!error <proflik: PNUM must be a scalar number indexing a non-fixed parameter.> ...
%! proflik (prob.GammaDistribution.fit (x), {1})
%!error <proflik: SETPARAM must be a numeric vector.> ...
%! proflik (prob.GammaDistribution.fit (x), 1, ones (2))
%!error <proflik: missing VALUE for 'Display' argument.> ...
%! proflik (prob.GammaDistribution.fit (x), 1, 'Display')
%!error <proflik: invalid VALUE type for 'Display' argument.> ...
%! proflik (prob.GammaDistribution.fit (x), 1, 'Display', 1)
%!error <proflik: invalid VALUE type for 'Display' argument.> ...
%! proflik (prob.GammaDistribution.fit (x), 1, 'Display', {1})
%!error <proflik: invalid VALUE type for 'Display' argument.> ...
%! proflik (prob.GammaDistribution.fit (x), 1, 'Display', {'on'})
%!error <proflik: invalid VALUE size for 'Display' argument.> ...
%! proflik (prob.GammaDistribution.fit (x), 1, 'Display', ['on'; 'on'])
%!error <proflik: invalid VALUE for 'Display' argument.> ...
%! proflik (prob.GammaDistribution.fit (x), 1, 'Display', 'onnn')
%!error <proflik: invalid NAME for optional arguments.> ...
%! proflik (prob.GammaDistribution.fit (x), 1, 'NAME', 'on')
%!error <proflik: invalid optional argument.> ...
%! proflik (prob.GammaDistribution.fit (x), 1, {'NAME'}, 'on')
%!error <proflik: invalid optional argument.> ...
%! proflik (prob.GammaDistribution.fit (x), 1, {[1 2 3 4]}, 'Display', 'on')

## 'truncate' method
%!error <truncate: missing input argument.> ...
%! truncate (prob.GammaDistribution)
%!error <truncate: missing input argument.> ...
%! truncate (prob.GammaDistribution, 2)
%!error <truncate: invalid lower upper limits.> ...
%! truncate (prob.GammaDistribution, 4, 2)

## Catch errors when using array of probability objects with available methods
%!shared pd
%! pd = prob.GammaDistribution (1, 1);
%! pd(2) = prob.GammaDistribution (1, 3);
...
%! pdfit = prob.GammaDistribution.fit (xdat');
