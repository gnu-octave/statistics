...
classdef ExtremeValueDistribution < prob.ProbabilityDistribution
  ## -*- texinfo -*-
  ## @deftp {statistics} prob.ExtremeValueDistribution
  ##
  ...
  properties(Dependent = true)
    ## -*- texinfo -*-
    ## @deftp {prob.ExtremeValueDistribution} {property} mu
    ##
    ## Location parameter
    ##
    ## A scalar value characterizing the location of the
    ## extreme value distribution. You can access the @qcode{mu}
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.ExtremeValueDistribution} {property} sigma
    ##
    ## Scale parameter
    ##
    ## A positive scalar value characterizing the scale of the
    ## extreme value distribution. You can access the @qcode{sigma}
    ## property using dot name assignment.
    ##
    ## @end deftp
    sigma
  ...
  properties(GetAccess = public, Constant = true)
    ## -*- texinfo -*-
    ## @deftp {prob.ExtremeValueDistribution} {property} DistributionName
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.ExtremeValueDistribution} {property} NumParameters
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.ExtremeValueDistribution} {property} ParameterNames
    ##
    ## Names of parameters
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.ExtremeValueDistribution} {property} ParameterDescription
    ##
  ...
  properties(GetAccess = public , SetAccess = protected)
    ## -*- texinfo -*-
    ## @deftp {prob.ExtremeValueDistribution} {property} ParameterValues
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.ExtremeValueDistribution} {property} ParameterCovariance
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.ExtremeValueDistribution} {property} ParameterIsFixed
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.ExtremeValueDistribution} {property} Truncation
    ##
    ## Truncation interval
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.ExtremeValueDistribution} {property} IsTruncated
    ##
    ...
    ## -*- texinfo -*-
    ## @deftp {prob.ExtremeValueDistribution} {property} InputData
    ##
  ...
  methods(Access = public)
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {@var{p} =} cdf (@var{pd}, @var{x})
    ## @deftypefnx {prob.ExtremeValueDistribution} {@var{p} =} cdf (@var{pd}, @var{x}, @qcode{'upper'})
    ##
    ...
    function p = cdf (this, x, uflag)
      ...
      if (this.IsTruncated)
        ...
        p(lb) = 0;
        p(ub) = 1;
        p(! (lb | ub)) -= evcdf (lx, this.mu, this.sigma);
        p(! (lb | ub)) /= diff (evcdf ([lx, ux], this.mu, this.sigma));
      endif
      ## Apply uflag
      if (utail)
        p = 1 - p;
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {@var{x} =} icdf (@var{pd}, @var{p})
    ##
    ...
    function x = icdf (this, p)
      ...
      if (this.IsTruncated)
        lp = evcdf (this.Truncation(1), this.mu, this.sigma);
        up = evcdf (this.Truncation(2), this.mu, this.sigma);
        ## Adjust p values within range of p @ lower limit and p @ upper limit
        is_nan = p < 0 | p > 1;
        p(is_nan) = NaN;
        np = lp + (up - lp) .* p;
        x = evinv (np, this.mu, this.sigma);
        x(x < this.Truncation(1)) = this.Truncation(1);
        x(x > this.Truncation(2)) = this.Truncation(2);
      else
        x = evinv (p, this.mu, this.sigma);
      endif
    endfunction

    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {@var{r} =} iqr (@var{pd})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {@var{m} =} mean (@var{pd})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {@var{m} =} median (@var{pd})
    ##
    ...
    function m = median (this)
      ...
      if (this.IsTruncated)
        lx = this.Truncation(1);
        ux = this.Truncation(2);
        Fa_b = evcdf ([lx, ux], this.mu, this.sigma);
        m = evinv (sum (Fa_b) / 2, this.mu, this.sigma);
      else
        m = evinv (0.5, this.mu, this.sigma);
      endif
    endfunction

    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {@var{nlogL} =} negloglik (@var{pd})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {@var{ci} =} paramci (@var{pd})
    ## @deftypefnx {prob.ExtremeValueDistribution} {@var{ci} =} paramci (@var{pd}, @var{Name}, @var{Value})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {@var{y} =} pdf (@var{pd}, @var{x})
    ##
    ...
    function y = pdf (this, x)
      ...
      if (this.IsTruncated)
        ...
        ub = x > ux;
        y(lb | ub) = 0;
        y(! (lb | ub)) /= diff (evcdf ([lx, ux], this.mu, this.sigma));
      endif
    endfunction

    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {} plot (@var{pd})
    ## @deftypefnx {prob.ExtremeValueDistribution} {} plot (@var{pd}, @var{Name}, @var{Value})
    ## @deftypefnx {prob.ExtremeValueDistribution} {@var{h} =} plot (@dots{})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd}, @var{pnum})
    ## @deftypefnx {prob.ExtremeValueDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd}, @var{pnum}, @qcode{'Display'}, @var{display})
    ## @deftypefnx {prob.ExtremeValueDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd}, @var{pnum}, @var{setparam})
    ## @deftypefnx {prob.ExtremeValueDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd}, @var{pnum}, @var{setparam}, @qcode{'Display'}, @var{display})
    ## @deftypefnx {prob.ExtremeValueDistribution} {[@var{nlogL}, @var{param}] =} proflik (@var{pd})
    ## @deftypefnx {prob.ExtremeValueDistribution} {[@var{nlogL}, @var{param}, @var{other}] =} proflik (@dots{})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {@var{r} =} random (@var{pd})
    ## @deftypefnx {prob.ExtremeValueDistribution} {@var{r} =} random (@var{pd}, @var{rows})
    ## @deftypefnx {prob.ExtremeValueDistribution} {@var{r} =} random (@var{pd}, @var{rows}, @var{cols}, @dots{})
    ## @deftypefnx {prob.ExtremeValueDistribution} {@var{r} =} random (@var{pd}, [@var{sz}])
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {@var{t} =} truncate (@var{pd}, @var{lower}, @var{upper})
    ##
    ...
    ## -*- texinfo -*-
    ## @deftypefn  {prob.ExtremeValueDistribution} {@var{v} =} var (@var{pd})
    ##
  ...
  methods(Static, Hidden)
    ...
    function pd = fit (x, varargin)
      ...
      [~, acov] = evlike (phat, x, censor, freq);
      ## Create fitted distribution object
      pd = prob.ExtremeValueDistribution.makeFitted ...
...
## Test output
%!shared pd, t
%! pd = prob.ExtremeValueDistribution (0, 1);
%! t = truncate (pd, 2, 4);
...
## 'prob.ExtremeValueDistribution' constructor
%!error <ExtremeValueDistribution: MU must be a real scalar.> ...
%! prob.ExtremeValueDistribution (Inf, 1)
%!error <ExtremeValueDistribution: MU must be a real scalar.> ...
%! prob.ExtremeValueDistribution (i, 1)
%!error <ExtremeValueDistribution: MU must be a real scalar.> ...
%! prob.ExtremeValueDistribution ('a', 1)
%!error <ExtremeValueDistribution: MU must be a real scalar.> ...
%! prob.ExtremeValueDistribution ([1, 2], 1)
%!error <ExtremeValueDistribution: MU must be a real scalar.> ...
%! prob.ExtremeValueDistribution (NaN, 1)
%!error <ExtremeValueDistribution: SIGMA must be a positive real scalar.> ...
%! prob.ExtremeValueDistribution (1, 0)
%!error <ExtremeValueDistribution: SIGMA must be a positive real scalar.> ...
%! prob.ExtremeValueDistribution (1, -1)
%!error <ExtremeValueDistribution: SIGMA must be a positive real scalar.> ...
%! prob.ExtremeValueDistribution (1, Inf)
%!error <ExtremeValueDistribution: SIGMA must be a positive real scalar.> ...
%! prob.ExtremeValueDistribution (1, i)
%!error <ExtremeValueDistribution: SIGMA must be a positive real scalar.> ...
%! prob.ExtremeValueDistribution (1, 'a')
%!error <ExtremeValueDistribution: SIGMA must be a positive real scalar.> ...
%! prob.ExtremeValueDistribution (1, [1, 2])
%!error <ExtremeValueDistribution: SIGMA must be a positive real scalar.> ...
%! prob.ExtremeValueDistribution (1, NaN)

## 'cdf' method
%!error <cdf: invalid argument for upper tail.> ...
%! cdf (prob.ExtremeValueDistribution, 2, 'uper')
%!error <cdf: invalid argument for upper tail.> ...
%! cdf (prob.ExtremeValueDistribution, 2, 3)
...
%! paramci (prob.ExtremeValueDistribution.fit (x), 'alpha')
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), 'alpha', 0)
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), 'alpha', 1)
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), 'alpha', [0.5 2])
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), 'alpha', '')
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), 'alpha', {0.05})
%!error <paramci: invalid VALUE for 'Alpha' argument.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), ...
%!          'parameter', 'mu', 'alpha', {0.05})
%!error <paramci: invalid VALUE size for 'Parameter' argument.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), ...
%!          'parameter', {'mu', 'sigma', 'param'})
%!error <paramci: invalid VALUE size for 'Parameter' argument.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), 'alpha', 0.01, ...
%!          'parameter', {'mu', 'sigma', 'param'})
%!error <paramci: unknown distribution parameter.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), 'parameter', 'param')
%!error <paramci: unknown distribution parameter.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), 'alpha', 0.01, ...
%!          'parameter', 'param')
%!error <paramci: invalid NAME for optional argument.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), 'NAME', 'value')
%!error <paramci: invalid NAME for optional argument.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), 'alpha', 0.01, 'NAME', 'value')
%!error <paramci: invalid NAME for optional argument.> ...
%! paramci (prob.ExtremeValueDistribution.fit (x), 'alpha', 0.01, ...
...
%! plot (prob.ExtremeValueDistribution, 'Parent')
%!error <plot: invalid VALUE for 'PlotType' argument.> ...
%! plot (prob.ExtremeValueDistribution, 'PlotType', 12)
%!error <plot: invalid VALUE size for 'Parameter' argument.> ...
%! plot (prob.ExtremeValueDistribution, 'PlotType', {'pdf', 'cdf'})
%!error <plot: invalid VALUE for 'PlotType' argument.> ...
%! plot (prob.ExtremeValueDistribution, 'PlotType', 'pdfcdf')
%!error <plot: invalid VALUE for 'Discrete' argument.> ...
%! plot (prob.ExtremeValueDistribution, 'Discrete', 'pdfcdf')
%!error <plot: invalid VALUE for 'Discrete' argument.> ...
%! plot (prob.ExtremeValueDistribution, 'Discrete', [1, 0])
%!error <plot: invalid VALUE for 'Discrete' argument.> ...
%! plot (prob.ExtremeValueDistribution, 'Discrete', {true})
%!error <plot: invalid VALUE for 'Parent' argument.> ...
%! plot (prob.ExtremeValueDistribution, 'Parent', 12)
%!error <plot: invalid VALUE for 'Parent' argument.> ...
%! plot (prob.ExtremeValueDistribution, 'Parent', 'hax')
%!error <plot: invalid NAME for optional argument.> ...
%! plot (prob.ExtremeValueDistribution, 'invalidNAME', 'pdf')
%!error <plot: no fitted DATA to plot a probability plot.> ...
%! plot (prob.ExtremeValueDistribution, 'PlotType', 'probability')

## 'proflik' method
%!error <proflik: no fitted data available.> ...
%! proflik (prob.ExtremeValueDistribution, 2)
%!error <proflik: PNUM must be a scalar number indexing a non-fixed parameter.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), 3)
%!error <proflik: PNUM must be a scalar number indexing a non-fixed parameter.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), [1, 2])
%!error <proflik: PNUM must be a scalar number indexing a non-fixed parameter.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), {1})
%!error <proflik: SETPARAM must be a numeric vector.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), 1, ones (2))
%!error <proflik: missing VALUE for 'Display' argument.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), 1, 'Display')
%!error <proflik: invalid VALUE type for 'Display' argument.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), 1, 'Display', 1)
%!error <proflik: invalid VALUE type for 'Display' argument.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), 1, 'Display', {1})
%!error <proflik: invalid VALUE type for 'Display' argument.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), 1, 'Display', {'on'})
%!error <proflik: invalid VALUE size for 'Display' argument.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), 1, 'Display', ['on'; 'on'])
%!error <proflik: invalid VALUE for 'Display' argument.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), 1, 'Display', 'onnn')
%!error <proflik: invalid NAME for optional arguments.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), 1, 'NAME', 'on')
%!error <proflik: invalid optional argument.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), 1, {'NAME'}, 'on')
%!error <proflik: invalid optional argument.> ...
%! proflik (prob.ExtremeValueDistribution.fit (x), 1, {[1 2 3 4]}, 'Display', 'on')

## 'truncate' method
%!error <truncate: missing input argument.> ...
%! truncate (prob.ExtremeValueDistribution)
%!error <truncate: missing input argument.> ...
%! truncate (prob.ExtremeValueDistribution, 2)
%!error <truncate: invalid lower upper limits.> ...
%! truncate (prob.ExtremeValueDistribution, 4, 2)

## Catch errors when using array of probability objects with available methods
%!shared pd
%! pd = prob.ExtremeValueDistribution (1, 1);
%! pd(2) = prob.ExtremeValueDistribution (1, 3);
