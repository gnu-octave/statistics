...
function r = raylrnd (sigma, varargin)
  ...
  elseif (nargin > 2)
    ...
    if (any (notint))
      error ("raylrnd: dimensions must be integers.");
    endif
    sz = [varargin{:}];
  endif
  ...
  if (! isscalar (sigma) && ! isequal (size (sigma), size (ones (sz))))
...
%!assert_equal (size (raylrnd (1, -1)), [0, 0])
%!assert_equal (size (raylrnd (1, [2, -1, 2])), [2, 0, 2])
%!assert_equal (size (raylrnd (1, 2, -1, 5)), [2, 0, 5])
...
%!error<raylrnd: SZ must be a scalar or a row vector of integers.> ...
%! raylrnd (1, 1.2)
%!error<raylrnd: SZ must be a scalar or a row vector of integers.> ...
%! raylrnd (1, ones (2))
%!error<raylrnd: SZ must be a scalar or a row vector of integers.> ...
