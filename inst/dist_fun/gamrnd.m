...
function r = gamrnd (a, b, varargin)
  ...
  elseif (nargin > 3)
    ...
    if (any (notint))
      error ("gamrnd: dimensions must be integers.");
    endif
    sz = [varargin{:}];
  endif
...
## Test output
%!assert_equal (size (gamrnd (1, 1)), [1 1])
%!assert_equal (size (gamrnd (1, ones (2,1))), [2, 1])
%!assert_equal (size (gamrnd (1, ones (2,2))), [2, 2])
%!assert_equal (size (gamrnd (ones (2,1), 1)), [2, 1])
%!assert_equal (size (gamrnd (ones (2,2), 1)), [2, 2])
%!assert_equal (size (gamrnd (1, 1, 3)), [3, 3])
%!assert_equal (size (gamrnd (1, 1, [4, 1])), [4, 1])
%!assert_equal (size (gamrnd (1, 1, 4, 1)), [4, 1])
%!assert_equal (size (gamrnd (1, 1, 4, 1, 5)), [4, 1, 5])
%!assert_equal (size (gamrnd (1, 1, 0, 1)), [0, 1])
%!assert_equal (size (gamrnd (1, 1, 1, 0)), [1, 0])
%!assert_equal (size (gamrnd (1, 1, 1, 2, 0, 5)), [1, 2, 0, 5])
%!assert_equal (size (gamrnd (1, 1, [])), [0, 0])
%!assert_equal (size (gamrnd (1, 1, [2, 0, 2, 1])), [2, 0, 2])
%!assert_equal (size (gamrnd (1, 2, -1)), [0, 0])
%!assert_equal (size (gamrnd (1, 2, [2, -1, 2])), [2, 0, 2])
%!assert_equal (size (gamrnd (1, 2, 2, -1, 5)), [2, 0, 5])

## Test class of input preserved
%!assert_equal (class (gamrnd (1, 1)), "double")
%!assert_equal (class (gamrnd (1, single (1))), "single")
%!assert_equal (class (gamrnd (1, single ([1, 1]))), "single")
%!assert_equal (class (gamrnd (single (1), 1)), "single")
%!assert_equal (class (gamrnd (single ([1, 1]), 1)), "single")
...
%!error<gamrnd: SZ must be a scalar or a row vector of integers.> ...
%! gamrnd (1, 2, 1.2)
%!error<gamrnd: SZ must be a scalar or a row vector of integers.> ...
%! gamrnd (1, 2, ones (2))
%!error<gamrnd: SZ must be a scalar or a row vector of integers.> ...
