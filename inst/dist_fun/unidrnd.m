...
function r = unidrnd (N, varargin)
  ...
  elseif (nargin > 2)
    ...
    if (any (notint))
      error ("unidrnd: dimensions must be integers.");
    endif
    sz = [varargin{:}];
  endif
...
## Test output
%!assert_equal (size (unidrnd (2)), [1, 1])
%!assert_equal (size (unidrnd (ones (2, 1))), [2, 1])
%!assert_equal (size (unidrnd (ones (2, 2))), [2, 2])
%!assert_equal (size (unidrnd (1, 3)), [3, 3])
%!assert_equal (size (unidrnd (1, [4, 1])), [4, 1])
%!assert_equal (size (unidrnd (1, 4, 1)), [4, 1])
%!assert_equal (size (unidrnd (1, 4, 1)), [4, 1])
%!assert_equal (size (unidrnd (1, 4, 1, 5)), [4, 1, 5])
%!assert_equal (size (unidrnd (1, 0, 1)), [0, 1])
%!assert_equal (size (unidrnd (1, 1, 0)), [1, 0])
%!assert_equal (size (unidrnd (1, 1, 2, 0, 5)), [1, 2, 0, 5])
%!assert_equal (size (unidrnd (1, [])), [0, 0])
%!assert_equal (size (unidrnd (1, [2, 0, 2, 1])), [2, 0, 2])
%!assert_equal (size (unidrnd (1, -1)), [0, 0])
%!assert_equal (size (unidrnd (1, [2, -1, 2])), [2, 0, 2])
%!assert_equal (size (unidrnd (1, 2, -1, 5)), [2, 0, 5])
%!assert_equal (unidrnd (0, 1, 1), NaN)
%!assert_equal (unidrnd ([0, 0, 0], [1, 3]), [NaN, NaN, NaN])

## Test class of input preserved
%!assert_equal (class (unidrnd (2)), "double")
%!assert_equal (class (unidrnd (single (2))), "single")
%!assert_equal (class (unidrnd (single ([2, 2]))), "single")

## Test input validation
%!error<unidrnd: function called with too few input arguments.> unidrnd ()
%!error<unidrnd: N must not be complex.> unidrnd (i)
%!error<unidrnd: SZ must be a scalar or a row vector of integers.> ...
%! unidrnd (1, 1.2)
%!error<unidrnd: SZ must be a scalar or a row vector of integers.> ...
%! unidrnd (1, ones (2))
%!error<unidrnd: SZ must be a scalar or a row vector of integers.> ...
