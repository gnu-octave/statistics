...
function r = hnrnd (mu, sigma, varargin)
  ...
  elseif (nargin > 3)
    ...
    if (any (notint))
      error ("hnrnd: dimensions must be integers.");
    endif
    sz = [varargin{:}];
  endif
...
## Test output
%!assert_equal (size (hnrnd (1, 1, 1)), [1, 1])
%!assert_equal (size (hnrnd (1, 1, 2)), [2, 2])
%!assert_equal (size (hnrnd (1, 1, [2, 1])), [2, 1])
%!assert_equal (size (hnrnd (1, zeros (2, 2))), [2, 2])
%!assert_equal (size (hnrnd (1, ones (2, 1))), [2, 1])
%!assert_equal (size (hnrnd (1, ones (2, 2))), [2, 2])
%!assert_equal (size (hnrnd (ones (2, 1), 1)), [2, 1])
%!assert_equal (size (hnrnd (ones (2, 2), 1)), [2, 2])
%!assert_equal (size (hnrnd (1, 1, 3)), [3, 3])
%!assert_equal (size (hnrnd (1, 1, [4, 1])), [4, 1])
%!assert_equal (size (hnrnd (1, 1, 4, 1)), [4, 1])
%!assert_equal (size (hnrnd (1, 1, [])), [0, 0])
%!assert_equal (size (hnrnd (1, 1, [2, 0, 2, 1])), [2, 0, 2])
%!assert_equal (size (hnrnd (1, 2, -1)), [0, 0])
%!assert_equal (size (hnrnd (1, 2, [2, -1, 2])), [2, 0, 2])
%!assert_equal (size (hnrnd (1, 2, 2, -1, 5)), [2, 0, 5])
%!test
%! r =  hnrnd (1, [1, 0, -1]);
%! assert_equal (r([2:3]), [NaN, NaN])

## Test class of input preserved
%!assert_equal (class (hnrnd (1, 0)), "double")
%!assert_equal (class (hnrnd (1, single (0))), "single")
%!assert_equal (class (hnrnd (1, single ([0, 0]))), "single")
%!assert_equal (class (hnrnd (1, single (1))), "single")
%!assert_equal (class (hnrnd (1, single ([1, 1]))), "single")
%!assert_equal (class (hnrnd (single (1), 1)), "single")
%!assert_equal (class (hnrnd (single ([1, 1]), 1)), "single")
...
%!error<hnrnd: SZ must be a scalar or a row vector of integers.> ...
%! hnrnd (1, 2, 1.2)
%!error<hnrnd: SZ must be a scalar or a row vector of integers.> ...
%! hnrnd (1, 2, ones (2))
%!error<hnrnd: SZ must be a scalar or a row vector of integers.> ...
